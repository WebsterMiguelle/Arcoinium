...
# ... [Keep your all_cards array exactly as it is] ...
var all_cards = [
...
func _populate_cards() -> void:
	for data in all_cards:
		...
		# Assuming setup doesn't strictly need a main node reference here, or you handle it in the scene.
		if card.has_method("setup"):
			card.setup(self) # Or pass whatever node setup expects
...
func _filter_by_rank(rank: String, pressed_button: Button) -> void:
	
	# If we clicked the button that is ALREADY the active filter...
	if current_filter == rank:
		current_filter = "" # Turn off the filter
		pressed_button.button_pressed = false # Un-press the button physically
	else:
		# If we clicked a NEW filter...
		current_filter = rank
		
		# Force all other buttons to un-press so only one is down at a time!
		if pressed_button != b_rank_btn: b_rank_btn.button_pressed = false
		if pressed_button != a_rank_btn: a_rank_btn.button_pressed = false
		if pressed_button != s_rank_btn: s_rank_btn.button_pressed = false
		
		# Ensure the one we just clicked stays down
		pressed_button.button_pressed = true 

	# Apply the visual filtering to the grid
