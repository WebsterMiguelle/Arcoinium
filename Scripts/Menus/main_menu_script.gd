...
@onready var tutorial_coin: AnimatedSprite2D = $Coin2
@onready var tutorial_button: Button = $Tutorial
@onready var tutorial_glow_panel: Panel = $Glow2
...
func _on_tutorial_frame_changed() -> void:
	if is_waiting_to_stop and tutorial_coin.frame == freeze_frame_on:
		tutorial_coin.pause()
...
func _tutorial_coin_float():
	...
	tutorial_float_tween.tween_property(tutorial_coin, "position:y", -20.0, 0.5).as_relative().set_trans(Tween.TRANS_SINE)
	tutorial_float_tween.parallel().tween_property(tutorial_glow_panel, "position:y", -20.0, 0.5).as_relative().set_trans(Tween.TRANS_SINE)
	tutorial_float_tween.parallel().tween_property(tutorial_button, "position:y", -20.0, 0.5).as_relative().set_trans(Tween.TRANS_SINE)

	tutorial_float_tween.tween_property(tutorial_coin, "position:y", 20.0, 0.5).as_relative().set_trans(Tween.TRANS_SINE)
	tutorial_float_tween.parallel().tween_property(tutorial_glow_panel, "position:y", 20.0, 0.5).as_relative().set_trans(Tween.TRANS_SINE)
	tutorial_float_tween.parallel().tween_property(tutorial_button, "position:y", 20.0, 0.5).as_relative().set_trans(Tween.TRANS_SINE)

func tutorial_coin_float():
	button.disabled = false
	if tutorial_float_tween:
		tutorial_float_tween.kill()
