...
func re_flip():
	...
	if has_refund and refund_chance <= 0.1:
		...
		current_re_flip = 0
		trigger_temp_passive("refund", "Reflip Refunded!")
	for c in coins:
		if !c.reserved:
			index += 1
		if index <= 2 and has_advanced_planning:
			...
			if has_refund and refund_chance <= 0.1:
				coin += 1
				trigger_temp_passive("refund", "+1 Coin!")
...
func start_turn():
	...
	if has_active_income and gain >= 30:
		trigger_temp_passive("active_income", "Active Income!")
	...
	if player_turn_count != 1:
		...
		for coin in coins:
			if coin.reserved:
				...
				check_solar_lunar_spawn(latest_coin)
				...
				if has_dividend and dividend_chance <= 0.3:
					...
					check_solar_lunar_spawn(dividend_coin)
					if has_simple_interest: gain += 1
					if has_withdraw:
						trigger_temp_passive("withdraw", "Withdraw DMG!")
						main.enemy.take_damage(1)
...
func check_solar_lunar_spawn(c):
	if c == null:
		return
		
	if c.state == 0 and has_solar_coin:
		trigger_temp_passive("solar_coin_spawn", "Solar Coin Appeared!")
		
	elif c.state == 1 and has_lunar_coin:
		trigger_temp_passive("lunar_coin_spawn", "Lunar Coin Appeared!")
...
func activate_player_turn_start_passives():
	...
	if payback_used and payback_coins != 0:
		trigger_temp_passive("payback", "Payback Activated!")
	...
	if latest_pair_left_coin != null and latest_pair_right_coin != null:
		trigger_temp_passive("piggy", "Piggy Trigger!")
		...
		add_child(left_coin);
		check_solar_lunar_spawn(left_coin)
		...
		check_solar_lunar_spawn(right_coin)
